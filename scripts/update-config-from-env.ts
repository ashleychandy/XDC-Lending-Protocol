 ...
 * or: tsx scripts/update-config-from-env.ts
 */
...
function generateChainConfig(envVars: EnvVars): string {
...
export interface ChainConfig {
  ...
  tokens: {
    // Wrapped native token (WXDC, WXDC, etc.)
    wxdc: {
      address: \`0x\${string}\`;
      symbol: string;
      decimals: number;
...
function main() {
  ...
  const configPath = path.join(process.cwd(), "src/config/chains.ts");
